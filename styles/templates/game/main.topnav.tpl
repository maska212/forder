...
{if $resouceData.used1 > $resouceData.max}
						...
						{/if}
							...
							<div class="stock_text tooltip" data-tooltip-content="Available {$resouceData.current|number}"><span style="color:{if $resouceData.current|number > 0}lime{elseif $resouceData.current|number < 0}red{else}white{/if}" id="current_{$resouceData.name}" name="{$resouceData.current|number}" data-real="{$resouceData.current}">{$resouceData.current|number}&nbsp;/&nbsp;{$resouceData.max|number}</span> </div>
                        </div>
                    </div>					
